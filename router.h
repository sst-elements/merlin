...
class BaseRtrEvent : public Event {
  ...
  protected:
    BaseRtrEvent(RtrEventType type) : Event(), type(type) {}
...
class RtrEvent : public BaseRtrEvent {
  ...
  public:
    ...
    RtrEvent(SST::Interfaces::SimpleNetwork::Request *req, SST::Interfaces::SimpleNetwork::nid_t trusted_src,
             int route_vn)
        : BaseRtrEvent(BaseRtrEvent::PACKET), request(req), trusted_src(trusted_src), route_vn(route_vn)
    ...
    inline void computeSizeInFlits(int flit_size) {
        size_in_flits = (request->size_in_bits + flit_size - 1) / flit_size;
    }
    ...
    void print(const std::string &header, Output &out) const override {
        out.output("%s RtrEvent to be delivered at %" PRIu64
                   " with priority %d. src = %lld (logical: %lld), dest = %lld\n",
...
class TopologyEvent : public BaseRtrEvent {
  ...
  public:
    TopologyEvent(int size_in_flits) : BaseRtrEvent(BaseRtrEvent::TOPOLOGY), size_in_flits(size_in_flits) {}
...
class credit_event : public BaseRtrEvent {
  public:
    ...
    credit_event(int vc, int credits) : BaseRtrEvent(BaseRtrEvent::CREDIT), vc(vc), credits(credits) {}
...
class internal_router_event : public BaseRtrEvent {
  ...
  public:
    ...
    internal_router_event(RtrEvent *ev) : BaseRtrEvent(BaseRtrEvent::INTERNAL) { encap_ev = ev; }

    ~internal_router_event() override {
        if (encap_ev != nullptr)
            delete encap_ev;
    }

    internal_router_event *clone() override { return new internal_router_event(*this); };
...
class PortInterface : public SubComponent {
  ...
  public:
    ...
    PortInterface(ComponentId_t cid) : SubComponent(cid) {}

    virtual void initVCs(int vns, int *vcs_per_vn, internal_router_event **vc_heads, int *xbar_in_credits,
                         int *output_queue_lengths) = 0;

    ~PortInterface() override = default;
    // void setup();
    // void finish();
    ...
    class OutputArbitration : public SubComponent {
      public:
        ...
        OutputArbitration(ComponentId_t cid) : SubComponent(cid) {}
        ~OutputArbitration() override = default;

        virtual void setVCs(int num_vns, int *vcs_per_vn) = 0;
        virtual int arbitrate(Cycle_t cycle, PortInterface::port_queue_t *out_q, int *port_out_credits, bool isHostPort,
                              bool &have_packets) = 0;
        virtual void dumpState(std::ostream &stream){};
    };
};
...
class XbarArbitration : public SubComponent {
  public:
    ...
    XbarArbitration(ComponentId_t cid) : SubComponent(cid) {}
    ~XbarArbitration() override = default;

#if VERIFY_DECLOCKING
    virtual void arbitrate(PortInterface **ports, int *port_busy, int *out_port_busy, int *progress_vc,
                           bool clocking) = 0;
#else
