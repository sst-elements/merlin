...
static std::string getLogicalGroupParam(const Params &params, Topology *topo, int port, std::string param,
...
static UnitAlgebra getLogicalGroupParamUA(const Params &params, Topology *topo, int port, std::string param,
                                          std::string default_val = "") {
...
hr_router::hr_router(ComponentId_t cid, Params &params)
    : Router(cid), num_vcs(-1), output(Simulation::getSimulation()->getSimulationOutput()) {
    ...
    // Get the topology
    topo = loadUserSubComponent<SST::Merlin::Topology>("topology", ComponentInfo::SHARE_NONE, num_ports, id);
    ...
    if (vn_remap_shm != "") {
        ...
        if (id == 0) {
            SharedRegion *sr = Simulation::getSharedRegionManager()->getGlobalSharedRegion(
                vn_remap_shm, vn_remap_shm_size, new SharedRegionMerger());
    ...
    if (pc_params.contains("network_inspectors"))
        pc_params.insert("network_inspectors", params.find<std::string>("network_inspectors", ""));
    ...
    for (int i = 0; i < num_ports; i++) {
        ...
        //                            inspector_names,
        // 						   std::stof(getLogicalGroupParam(params,topo,i,"dlink_thresh",
        // "-1")),
        //                            oql_track_port,oql_track_remote);

        ports[i] = loadAnonymousSubComponent<PortInterface>("merlin.portcontrol", "portcontrol", i,
    ...
    arb = loadAnonymousSubComponent<XbarArbitration>(xbar_arb, "XbarArb", 0, ComponentInfo::INSERT_STATS, empty_params);
    ...
    if (xbar_bw_ua < link_bw) {
        ...
        std::cout << "  xbar_bw = " << xbar_bw_ua.toStringBestSI() << ", link_bw = " << link_bw.toStringBestSI()
                  << std::endl;
        abort();
    }
    // Register statistics
...
bool hr_router::clock_handler(Cycle_t cycle) {
        ...
        if (progress_vcs[i] > -1) {
            ...
            if (ev->getTraceType() == SimpleNetwork::Request::FULL) {
                output.output("TRACE(%d): %" PRIu64 " ns: Copying event (src = %d, dest = %d) "
                              ...
                              " %d, VC %d.\n",
                              ev->getTraceID(), getCurrentSimTimeNano(), ev->getSrc(), ev->getDest(), id,
                              getName().c_str(), i, progress_vcs[i], ev->getNextPort(), ev->getVC());
            }
...
void hr_router::init_vcs() {
    ...
    for (int i = 0; i < num_ports; i++) {
        ports[i]->initVCs(num_vns, vcs_per_vn, &vc_heads[i * num_vcs], &xbar_in_credits[i * num_vcs],
