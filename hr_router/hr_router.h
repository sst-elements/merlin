...
class hr_router : public Router {
  ...
  public:
    SST_ELI_REGISTER_COMPONENT(hr_router, "merlin", "hr_router", SST_ELI_ELEMENT_VERSION(1, 0, 0), "High radix router",
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        {"input_latency",
         "Latency of packets entering switch into input buffers.  Specified in s (can include SI prefix)."},
        {"output_latency",
         "Latency of packets exiting switch from output buffers.  Specified in s (can include SI prefix)."},
        ...
        {"oql_track_port", "Set to true to track output queue length for an entire port.  False tracks per VC.",
         "false"},
        {"oql_track_remote",
         "Set to true to track output queue length including remote input queue.  False tracks only local queue.",
         "false"},
    ...
    SST_ELI_DOCUMENT_PORTS({"port%(num_ports)d",
                            ...
                            {"merlin.RtrEvent", "merlin.internal_router_event", "merlin.topologyevent",
                             "merlin.credit_event"}})

    SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS({"topology", "Topology object to control routing", "SST::Merlin::Topology"},
