...
class xbar_arb_age : public XbarArbitration {
   public:
    SST_ELI_REGISTER_SUBCOMPONENT(xbar_arb_age, "merlin", "xbar_arb_age",
   ...
   private:
    ...
    struct priority_entry_t {
        ...
        priority_entry_t(uint16_t port, uint16_t vc)
            : port(port), vc(vc), next_port(0), next_vc(0), injection_time(0) {}
    };
    ...
    class time_priority {
       public:
        ...
        inline auto operator()(const priority_entry_t *lhs, const priority_entry_t *rhs) const
            -> bool {
            return lhs->injection_time > rhs->injection_time;
        }

        /** Compare based off references */
        inline auto operator()(const priority_entry_t &lhs, const priority_entry_t &rhs) -> bool {
            return lhs.injection_time > rhs.injection_time;
        }
    };
    ...
    typedef std::priority_queue<priority_entry_t *, std::vector<priority_entry_t *>,
                                xbar_arb_age::time_priority>
        age_queue_t;
   ...
   public:
    xbar_arb_age(Component *parent, Params & /*params*/) : XbarArbitration(parent) {}
        ...
        for (int i = 0; i < num_ports; i++) {
            ...
            for (int j = 0; j < num_vcs; j++) {
                ...
                if (src_event != nullptr) {
                    ...
                    entries[index].injection_time =
                        vc_heads[j]->getEncapsulatedEvent()->getInjectionTime();
