...
class xbar_arb_rand : public XbarArbitration {
   public:
    SST_ELI_REGISTER_SUBCOMPONENT(xbar_arb_rand, "merlin", "xbar_arb_rand",
   ...
   private:
    ...
    struct priority_entry_t {
        ...
        priority_entry_t(uint16_t port, uint16_t vc)
            : port(port), vc(vc), next_port(0), next_vc(0) {}
    };
   ...
   public:
    xbar_arb_rand(Component *parent, Params & /*params*/) : XbarArbitration(parent) {
        rng = new RNG::XORShiftRNG(69);
    }

    ~xbar_arb_rand() override { delete[] entries; }
