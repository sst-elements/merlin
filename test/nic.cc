...
nic::nic(ComponentId_t cid, Params &params)
    : Component(cid), packets_sent(0), packets_recd(0), stalled_cycles(0), expected_recv_count(0), done(false),
      initialized(false), init_state(0), init_count(0), init_broadcast_count(0),
...
void nic::setup() {
    ...
    if (link_control->getEndpointID() != net_id) {
        output.output("NIC ids don't match: param = %" PRIi64 ", LinkControl = %" PRIi64 "\n", (int64_t)net_id,
                      (int64_t)link_control->getEndpointID());
    }
...
void nic::init_complete(unsigned int /*phase*/) {
    ...
    case 0: {
        ...
        if (net_id == 0) {
            if (send_untimed_bcast) {
                auto *req = new SimpleNetwork::Request(SimpleNetwork::INIT_BROADCAST_ADDR, net_id, 0, true, true);
                ...
                for (int i = 0; i < num_peers; ++i) {
                    auto *req = new SimpleNetwork::Request(i, net_id, 0, true, true);
                    link_control->sendUntimedData(req);
                }
                init_state = 4;
            }
    ...
    case 3: {
        ...
        while ((req = link_control->recvInitData()) != nullptr) {
            ...
            // if ( req->dest != net_id ) output.output("%d: received event with dest %lld and src
            // %lld\n",net_id,req->dest,req->src);
            init_count++;
            delete req;
        }
...
class MyRtrEvent : public Event {
  public:
    ...
    Event *clone() override { return new MyRtrEvent(*this); }
    ...
    void print(const std::string &header, Output &out) const override {
        out.output("%s MyRtrEvent to be delivered at %" PRIu64 " with priority %d.  seq = %d\n", header.c_str(),
                   getDeliveryTime(), getPriority(), seq);
    }
...
bool nic::clock_handler(Cycle_t cycle) {
