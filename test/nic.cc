...
nic::nic(ComponentId_t cid, Params &params)
    ...
    if (message_size.hasUnits("B")) {
        message_size *= UnitAlgebra("8b/B");
    }
    msg_size = message_size.getRoundedValue();

    std::string linkcontrol_type =
        params.find<std::string>("linkcontrol_type", "merlin.linkcontrol");
    // Create a LinkControl object
    // NOTE:  This MUST be the same length as 'num_vns'
    link_control = dynamic_cast<SimpleNetwork *>(loadSubComponent(linkcontrol_type, this, params));
...
void nic::setup() {
    ...
    if (link_control->getEndpointID() != net_id) {
        output.output("NIC ids don't match: param = %" PRIi64 ", LinkControl = %" PRIi64 "\n",
                      static_cast<int64_t>(net_id),
                      static_cast<int64_t>(link_control->getEndpointID()));
    }
...
void nic::init_complete(unsigned int /*phase*/) {
    switch (init_state) {
        case 0: {
            ...
            if (net_id == 0) {
                ...
                auto *req = new SimpleNetwork::Request(SimpleNetwork::INIT_BROADCAST_ADDR, net_id,
                                                       0, true, true);
        ...
        case 1: {
            ...
            while ((req = link_control->recvInitData()) != nullptr) {
                // std::cout << "NIC " << id << " Received an init event from " << req->src << " in
        ...
        case 3: {
            SimpleNetwork::Request *req;
            while ((req = link_control->recvInitData()) != nullptr) {
                // std::cout << "NIC " << id << " Received an init event in phase " << phase << "!"
                // << std::endl; Only point to points could arrive now if ( req->dest != net_id )
                // output.output("%d: received event with dest %lld and src
...
class MyRtrEvent : public Event {
   public:
    ...
    explicit MyRtrEvent(int seq) : seq(seq) {}

    auto clone() -> Event * override { return new MyRtrEvent(*this); }
...
//         expected_recv_count); primaryComponentOKToEndSim(); done = true;
//     }
//     // Send packets
...
//                 delete req;
//             }
//             //std::cout << cycle << ": " << id << " sent packet " << ev->seq << " to " <<
//             ev->dest << std::endl; packets_sent++; if ( packets_sent == expected_recv_count ) {
...
//             if ( req->dest != net_id ) {
//                 output.fatal(CALL_INFO,-1,"%d received packet intended for
//                 %d\n",net_id,(int)req->dest);
//             }
//             else {
...
// #endif
//             next_seq[src]++;
//             //std::cout << cycle << ": " << id << " Received an event on vn " << rec_ev->vn << "
//             from " << rec_ev->src << " (packet "<<packets_recd<<" )"<< std::endl; delete ev;
//             delete req;
//             break;
//         }
//     }
...
auto nic::clock_handler(Cycle_t cycle) -> bool {
    static const int send_vc = 0;
    ...
    if (packets_sent < expected_recv_count) {
        if (link_control->spaceToSend(send_vc, msg_size)) {
            ...
            auto *ev = new MyRtrEvent(packets_sent / num_peers);
            ...
            // output.output("(%lld) %d: sent packet to
    ...
    for (int vn = 0; vn < num_vns; vn++) {
        ...
        if (link_control->requestToReceive(last_vn)) {
            SimpleNetwork::Request *req = link_control->recv(last_vn);
            auto *ev = dynamic_cast<MyRtrEvent *>(req->takePayload());
            if (ev == nullptr) {
                Simulation::getSimulation()->getSimulationOutput().fatal(CALL_INFO, -1,
                                                                         "Aieeee!\n");
            }
            ...
            next_seq[src]++;
            // std::cout << cycle << ": " << id << " Received an event on vn " << rec_ev->vn << "
            // from " << rec_ev->src << " (packet "<<packets_recd<<" )"<< std::endl;
            delete ev;
            delete req;
            break;
        }
