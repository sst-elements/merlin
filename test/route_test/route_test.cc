...
route_test::route_test(ComponentId_t cid, Params &params)
    ...
    link_control =
        dynamic_cast<SimpleNetwork *>(loadSubComponent("merlin.linkcontrol", this, params));
    ...
    // // Register a clock
    // registerClock( "1GHz", new Clock::Handler<route_test>(this,&route_test::clock_handler),
    // false);
...
void route_test::init(unsigned int phase) {
    ...
    if (id == 0 && !initialized) {
        if (link_control->isNetworkInitialized()) {
            ...
            auto *req =
                new SimpleNetwork::Request(SimpleNetwork::INIT_BROADCAST_ADDR, id, 0, true, true);
            link_control->sendInitData(req);
        }
    } else {
        SimpleNetwork::Request *req = link_control->recvInitData();
        if (req != nullptr) {
            // std::cout << "ROUTE_TEST " << id << " Received an init event in phase " << phase <<
            // "!" << std::endl;
            delete req;
            initialized = true;
        }
...
void route_test::setup() {
    ...
    if (link_control->getEndpointID() != id) {
        std::cout << "NIC ids don't match: param = " << id
                  << ", LinkControl = " << link_control->getEndpointID() << std::endl;
    }
    if (!initialized) {
...
auto route_test::handle_event(int vn) -> bool {
