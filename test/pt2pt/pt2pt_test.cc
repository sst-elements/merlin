...
pt2pt_test::pt2pt_test(ComponentId_t cid, Params &params) : Component(cid), id(-1), packets_sent(0) {
    ...
    if (!packet_size_ua.hasUnits("b") && !packet_size_ua.hasUnits("B")) {
        merlin_abort.fatal(CALL_INFO, -1,
                           "packet_size must be specified in either "
                           "bits or bytes: %s\n",
                           packet_size_ua.toStringBestSI().c_str());
    }
    if (packet_size_ua.hasUnits("B"))
        packet_size_ua *= UnitAlgebra("8b/B");
    ...
    if (buffer_size.hasUnits("B"))
        buffer_size *= UnitAlgebra("8b/B");
    ...
    if (delays.size() != src.size()) {
        merlin_abort.fatal(CALL_INFO, -1,
                           "pt2pt_test: stream_delays array must be same length as \"src\" and \"dest\" arrays\n");
    }

    self_link = configureSelfLink("start_timing", "1ns", new Event::Handler<pt2pt_test>(this, &pt2pt_test::start));

    report_timing =
        configureSelfLink("report_timing", "1ns", new Event::Handler<pt2pt_test>(this, &pt2pt_test::report_bw));
...
void pt2pt_test::finish() {
    ...
    for (auto &x : my_recvs) {
        ...
        UnitAlgebra total_time =
            Simulation::getSimulation()->getTimeLord()->getTimeBase() * (x.second.end_arrival - x.second.first_arrival);

        UnitAlgebra bw = bits_sent / total_time;

        Simulation::getSimulationOutput().output("For src = %d and dest = %d:\n"
                                                 "  First packet received at: %s\n"
                                                 "  Last packet received at: %s\n"
                                                 "  Bandwidth: %s (%s)\n\n",
                                                 x.first, id, start_time.toStringBestSI().c_str(),
                                                 end_time.toStringBestSI().c_str(), bw.toStringBestSI().c_str(),
                                                 (bw / UnitAlgebra("8 b/B")).toStringBestSI().c_str());
    }
}
...
void pt2pt_test::setup() {
    ...
    if (my_dest != -1) {
        ...
        self_link->send(delay_in_ns.getRoundedValue(), nullptr);
    }
...
void pt2pt_test::init(unsigned int phase) {
...
bool pt2pt_test::send_handler(int /*vn*/) {
    ...
    while (link_control->spaceToSend(0, packet_size) && packets_sent < packets_to_send) {
        ...
        link_control->send(req, 0);
        // std::cout << id << ": Sending packet to: " << my_dest << " at " <<
...
bool pt2pt_test::recv_handler(int /*vn*/) {
    ...
    // std::cout << "Recived packet from: " << src << " at " << Simulation::getSimulation()->getCurrentSimCycle() <<
    // std::endl;
