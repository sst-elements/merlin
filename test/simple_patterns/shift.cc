...
shift_nic::shift_nic(ComponentId_t cid, Params &params)
    : Component(cid), send_seq(0), recv_seq(0), num_ooo(0), packets_sent(0), packets_recd(0), stalled_cycles(0),
      send_done(false), recv_done(false), initialized(false),
    ...
    if (!link_control) {
        ...
        link_control = loadAnonymousSubComponent<SST::Interfaces::SimpleNetwork>(
            "merlin.reorderlinkcontrol", "networkIF", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS,
            if_params, 1 /* vns */);
    }
...
class ShiftEvent : public Event {
  public:
    ...
    Event *clone() override { return new ShiftEvent(*this); }
...
bool shift_nic::clock_handler(Cycle_t cycle) {
...
bool shift_nic::handle_event(int /*vn*/) {
