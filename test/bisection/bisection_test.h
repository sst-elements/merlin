...
class bisection_test : public Component {
   public:
    SST_ELI_REGISTER_COMPONENT(bisection_test, "merlin", "bisection_test",
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        {"buffer_size",
         "Size of input and output buffers specified in b or B (can include SI prefix).", "128B"},
        {"networkIF", "Network interface to use.  Must inherit from SimpleNetwork",
         "merlin.linkcontrol"})
   ...
   public:
    ...
    ~bisection_test() override = default;
   ...
   private:
    ...
    auto receive_handler(int vn) -> bool;

    auto send_handler(int vn) -> bool;
};
...
class bisection_test_event : public Event {
   public:
    ...
    bisection_test_event() = default;

    auto clone() -> Event * override { return new bisection_test_event(*this); }
