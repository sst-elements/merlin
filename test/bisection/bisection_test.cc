...
bisection_test::bisection_test(ComponentId_t cid, Params &params) : Component(cid), packets_sent(0), packets_recd(0) {
    ...
    link_control->setNotifyOnReceive(
        new SST::Interfaces::SimpleNetwork::Handler<bisection_test>(this, &bisection_test::receive_handler));

    // Set up a send functor that will handle sending packets
    link_control->setNotifyOnSend(
        new SST::Interfaces::SimpleNetwork::Handler<bisection_test>(this, &bisection_test::send_handler));
...
bool bisection_test::send_handler(int vn) {
...
bool bisection_test::receive_handler(int vn) {
    ...
    while (link_control->requestToReceive(vn)) {
        ...
        auto *rec_ev = static_cast<bisection_test_event *>(req->takePayload());
