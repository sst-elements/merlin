...
bisection_test::bisection_test(ComponentId_t cid, Params &params)
    : Component(cid), packets_sent(0), packets_recd(0) {
    ...
    link_control =
        dynamic_cast<SST::Interfaces::SimpleNetwork *>(loadSubComponent(networkIF, this, params));
    ...
    link_control->setNotifyOnReceive(new SST::Interfaces::SimpleNetwork::Handler<bisection_test>(
        this, &bisection_test::receive_handler));

    // Set up a send functor that will handle sending packets
    link_control->setNotifyOnSend(new SST::Interfaces::SimpleNetwork::Handler<bisection_test>(
        this, &bisection_test::send_handler));
...
auto bisection_test::send_handler(int vn) -> bool {
    ...
    while (link_control->spaceToSend(vn, packet_size) && (packets_sent < packets_to_send)) {
        auto *ev = new bisection_test_event();
...
auto bisection_test::receive_handler(int vn) -> bool {
    ...
    while (link_control->requestToReceive(vn)) {
        ...
        auto *rec_ev = dynamic_cast<bisection_test_event *>(req->takePayload());
...
void bisection_test::finish() { link_control->finish(); }

void bisection_test::init(unsigned int phase) { link_control->init(phase); }

void bisection_test::handle_complete(Event *ev) {
    delete ev;
    ...
    double total_time = static_cast<double>(end_time) - double(start_time);
