...
LinkControl::LinkControl(ComponentId_t cid, Params &params, int vns)
    ...
    if (out_buf_size.hasUnits("B")) {
        out_buf_size *= UnitAlgebra("8b/B");
    }
...
void LinkControl::init(unsigned int phase) {
    ...
    switch (phase) {
        ...
        case 1: {
            ...
            if (dynamic_cast<BaseRtrEvent *>(ev)->getType() != BaseRtrEvent::INITIALIZATION) {
                // fail
            }
            init_ev = dynamic_cast<RtrInitEvent *>(ev);
        ...
        default:
            ...
            while ((ev = rtr_link->recvInitData()) != nullptr) {
                auto *bev = dynamic_cast<BaseRtrEvent *>(ev);
                switch (bev->getType()) {
                    ...
                    case BaseRtrEvent::PACKET:
                        init_events.push_back(dynamic_cast<RtrEvent *>(ev));
                        break;
                    default:
                        // This shouldn't happen.  Only RtrEvents (PACKET
                        // types) should not be handled in the LinkControl
                        // object.
                        merlin_abort_full.fatal(
                            CALL_INFO, 1, "Reached state where a non-RtrEvent was not handled.");
                        break;
                }
            }
            break;
...
auto LinkControl::recv(int vn) -> SST::Interfaces::SimpleNetwork::Request * {
    ...
    if (event->getTraceType() != SimpleNetwork::Request::NONE) {
        output.output("TRACE(%d): %" PRIu64 " ns: recv called on LinkControl in NIC: %s\n",
                      event->getTraceID(), getCurrentSimTimeNano(), getName().c_str());
    }
...
auto LinkControl::recvUntimedData() -> SST::Interfaces::SimpleNetwork::Request * {
...
void LinkControl::handle_input(Event *ev) {
    ...
    if (base_event->getType() == BaseRtrEvent::CREDIT) {
        ...
        rtr_credits[ce->vc] += ce->credits;
        // std::cout << "Got " << ce->credits << " credits for VN: " << ce->vc << ".  Current
        // credits: " << rtr_credits[ce->vc] << std::endl;
        delete ev;
        ...
        // event->request->src, event->request->vn, actual_vn); std::cout << std::endl;
        ...
        if (event->request->getTraceType() == SimpleNetwork::Request::FULL) {
            output.output("TRACE(%d): %" PRIu64
                          ...
                          event->request->getTraceID(), getCurrentSimTimeNano(), getName().c_str(),
                          event->request->vn, event->request->src);
        }
...
void LinkControl::handle_output(Event * /*ev*/) {
    ...
    for (int i = curr_out_vn; i < total_vns; i++) {
        ...
        if (rtr_credits[i] < send_event->getSizeInFlits()) {
            continue;
        }
        vn_to_send = i;
        output_buf[i].pop();
