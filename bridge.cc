...
Bridge::Bridge(SST::ComponentId_t id, SST::Params &params) : SST::Component(id) {
    int debugLevel = params.find<int>("debug_level", 0);
    dbg.init("@t:Bridge::@p():@l " + getName() + ": ", debugLevel, 0,
             static_cast<Output::output_location_t>(params.find<int>("debug", 0)));

    Params transParams = params.find_prefix_params("translator.");
    translator = dynamic_cast<Translator *>(
        loadSubComponent(params.find<std::string>("translator"), this, transParams));
    if (translator == nullptr) {
        dbg.fatal(CALL_INFO, 1, "Must specify a 'translator' subcomponent.");
    }
...
void Bridge::configureNIC(uint8_t id, SST::Params &params) {
    ...
    nic.nic = dynamic_cast<SimpleNetwork *>(loadSubComponent("merlin.linkcontrol", this, params));
    nic.nic->initialize(
...
auto Bridge::handleIncoming(int vn, uint8_t id) -> bool {
...
auto Bridge::spaceAvailable(int /*vn*/, uint8_t id) -> bool {
    Nic_t &nic = interfaces[id];
