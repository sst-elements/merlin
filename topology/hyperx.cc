...
void topo_hyperx::route(int port, int vc, internal_router_event *ev) {
    ...
    auto *tt_ev = static_cast<topo_hyperx_event *>(ev);
...
internal_router_event *topo_hyperx::process_input(RtrEvent *ev) {
...
Topology::PortState topo_hyperx::getPortState(int port) const {
    if (port >= local_port_start) {
...
int topo_hyperx::get_dest_router(int dest_id) const { return dest_id / num_local_ports; }

// dest_id is a host id
int topo_hyperx::get_dest_local_port(int dest_id) const { return local_port_start + (dest_id % num_local_ports); }

int topo_hyperx::choose_multipath(int start_port, int /*num_ports*/) {
...
int topo_hyperx::computeNumVCs(int vns) { return vcs_per_vn * vns; }

int topo_hyperx::getEndpointID(int port) {
    if (!isHostPort(port))
        return -1;
...
std::pair<int, int> topo_hyperx::routeDORBase(int *dest_loc) {
        ...
        for (int dim = 0; dim < dimensions; dim++) {
            ...
            if ((vc & 0x1) == 1) {
                ...
                for (int curr_port = port_start[dim];
                     curr_port < port_start[dim] + ((dim_size[dim] - 1) * dim_width[dim]); ++curr_port) {
...
void topo_hyperx::routeVDAL(int port, int vc, topo_hyperx_event *ev) {
    ...
    // trace.getOutput().output("%llu: udims.size = %lu, remaining_vcs = %d\n",ev->id.first,udims.size(),num_vcs -
    // start_vc - 1 ); Check to see if there are extra VCs for misroutes.  If not, simply fall back to MIN-A routing
