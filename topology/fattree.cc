...
void topo_fattree::parseShape(const std::string &shape, int *downs, int *ups) const {
    size_t start = 0;
    size_t end = 0;
...
topo_fattree::topo_fattree(ComponentId_t cid, Params &params, int num_ports, int rtr_id)
    : Topology(cid), id(rtr_id), num_ports(num_ports), num_vcs(-1), allow_adaptive(false) {
...
internal_router_event *topo_fattree::process_InitData_input(RtrEvent *ev) { return new internal_router_event(ev); }

int topo_fattree::getEndpointID(int port) { return low_host + port; }

Topology::PortState topo_fattree::getPortState(int port) const {
    if (rtr_level == 0) {
        if (port < down_ports)
            return R2N;
        else if (port >= down_ports)
            return R2R;
        else
            return UNCONNECTED;
    } else {
