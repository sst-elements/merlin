...
class topo_torus_event : public internal_router_event {
  public:
    ...
    topo_torus_event(int dim) {
        dimensions = dim;
        routing_dim = 0;
        dest_loc = new int[dim];
    }
    ~topo_torus_event() override { delete[] dest_loc; }
    internal_router_event *clone() override {
        auto *tte = new topo_torus_event(*this);
...
class topo_torus : public Topology {
  ...
  public:
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(topo_torus, "merlin", "torus", SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                          "Multi-dimensional torus topology object", SST::Merlin::Topology)

    SST_ELI_DOCUMENT_PARAMS(
        {"torus:shape", "Shape of the torus specified as the number of routers in each dimension, where each dimension "
                        "is separated by an x.  For example, 4x4x2x2.  Any number of dimensions is supported."},
