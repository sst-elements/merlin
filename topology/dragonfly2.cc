...
const RouterPortPair2 &RouteToGroup2::getRouterPortPair(int group, int route_number) {
 ...
 */
topo_dragonfly2::topo_dragonfly2(ComponentId_t cid, Params &p, int num_ports, int rtr_id) : Topology(cid) {
    ...
    SharedRegion *sr = Simulation::getSharedRegionManager()->getGlobalSharedRegion(
        "group_to_global_port", ((params.g - 1) * params.n) * sizeof(RouterPortPair2), new SharedRegionMerger());
...
void topo_dragonfly2::route(int port, int vc, internal_router_event *ev) {
    auto *td_ev = static_cast<topo_dragonfly2_event *>(ev);
...
int topo_dragonfly2::getEndpointID(int port) {
