...
void RouteToGroup::init(SharedRegion *sr, size_t g, size_t r) {
    region = sr;
...
auto RouteToGroup::getRouterPortPair(int group, int route_number) -> const RouterPortPair & {
    // data = static_cast<RouterPortPair*>(region->getRawPtr());
    return data[group * routes + route_number];
}

void RouteToGroup::setRouterPortPair(int group, int route_number, const RouterPortPair &pair) {
 ...
 */
topo_dragonfly::topo_dragonfly(Component *comp, Params &p) : Topology(comp) {
    ...
    SharedRegion *sr = Simulation::getSharedRegionManager()->getGlobalSharedRegion(
        "dragonfly:group_to_global_port", ((params.g - 1) * params.n) * sizeof(RouterPortPair),
...
topo_dragonfly::~topo_dragonfly() = default;

void topo_dragonfly::route(int port, int vc, internal_router_event *ev) {
    auto *td_ev = dynamic_cast<topo_dragonfly_event *>(ev);
...
auto topo_dragonfly::process_input(RtrEvent *ev) -> internal_router_event * {
    ...
    auto *td_ev = new topo_dragonfly_event(dstAddr);
...
auto topo_dragonfly::process_InitData_input(RtrEvent *ev) -> internal_router_event * {
    dgnflyAddr dstAddr{};
    idToLocation(ev->request->dest, &dstAddr);
    auto *td_ev = new topo_dragonfly_event(dstAddr);
    td_ev->src_group = group_id;
...
auto topo_dragonfly::getPortState(int port) const -> Topology::PortState {
    if (static_cast<uint32_t>(port) < params.p) {
        return R2N;
    }
...
auto topo_dragonfly::getPortLogicalGroup(int port) const -> std::string {
    if (static_cast<uint32_t>(port) < params.p) {
        return "host";
    }
    if (static_cast<uint32_t>(port) >= params.p &&
...
auto topo_dragonfly::getEndpointID(int port) -> int {
    return (group_id * (params.a /*rtr_per_group*/ * params.p /*hosts_per_rtr*/)) +
           (router_id * params.p /*hosts_per_rtr*/) + port;
}

void topo_dragonfly::setOutputBufferCreditArray(int const *array, int vcs) {
...
auto topo_dragonfly::router_to_group(uint32_t group) -> uint32_t {
    if (group < group_id) {
...
auto topo_dragonfly::port_for_group(uint32_t group, uint32_t slice, int /*id*/) -> uint32_t {
...
auto topo_dragonfly::port_for_router(uint32_t router) -> uint32_t {
