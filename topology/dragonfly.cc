...
const RouterPortPair &RouteToGroup::getRouterPortPair(int group, int route_number) {
...
topo_dragonfly::topo_dragonfly(ComponentId_t cid, Params &p, int num_ports, int rtr_id) : Topology(cid) {
    ...
    SharedRegion *sr = Simulation::getSharedRegionManager()->getGlobalSharedRegion(
        "group_to_global_port", ((params.g - 1) * params.n) * sizeof(RouterPortPair), new SharedRegionMerger());
...
void topo_dragonfly::route(int port, int vc, internal_router_event *ev) {
    auto *td_ev = static_cast<topo_dragonfly_event *>(ev);
        ...
        if (broadcast_to_groups) {
            for (int p = 0; p < (int)(params.g - 1); p++) {
                ...
                if (pair.router == router_id)
                    outPorts.push_back((int)(pair.port));
            }
        }
    } else {
...
int topo_dragonfly::getEndpointID(int port) {
