...
topo_torus::topo_torus(Component *comp, Params &params) : Topology(comp) {
    ...
    if (dest_router == router_id) {
        ...
        for (int dim = tt_ev->routing_dim; dim < dimensions; dim++) {
            if (tt_ev->dest_loc[dim] != id_loc[dim]) {
                ...
                if (dist_neg < 0) {
                    dist_neg += dim_size[dim];
                }
                int dist_pos = tt_ev->dest_loc[dim] - id_loc[dim];
                if (dist_pos < 0) {
                    dist_pos += dim_size[dim];
                }

                int go_pos = static_cast<int>(dist_pos <= dist_neg);
...
auto topo_torus::process_input(RtrEvent *ev) -> internal_router_event * {
    auto *tt_ev = new topo_torus_event(dimensions);
...
auto topo_torus::process_InitData_input(RtrEvent *ev) -> internal_router_event * {
    auto *tt_ev = new topo_torus_event(dimensions);
    tt_ev->setEncapsulatedEvent(ev);
    tt_ev->setVC(ev->request->vn * 2);
...
auto topo_torus::getPortState(int port) const -> Topology::PortState {
...
void topo_torus::idToLocation(int run_id, int *location) const {
...
void topo_torus::parseDimString(const std::string &shape, int *output) const {
    size_t start = 0;
    size_t end = 0;
    for (int i = 0; i < dimensions; i++) {
...
auto topo_torus::get_dest_router(int dest_id) const -> int { return dest_id / num_local_ports; }

auto topo_torus::get_dest_local_port(int dest_id) const -> int {
    return local_port_start + (dest_id % num_local_ports);
}

auto topo_torus::choose_multipath(int start_port, int num_ports, int dest_dist) -> int {
    if (num_ports == 1) {
        return start_port;
    }
...
auto topo_torus::computeNumVCs(int vns) -> int { return 2 * vns; }

auto topo_torus::getEndpointID(int port) -> int {
    if (!isHostPort(port)) {
        return -1;
    }
