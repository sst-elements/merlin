...
class RouteToGroup2 {
   ...
   public:
    ...
    auto getRouterPortPair(int group, int route_number) -> const RouterPortPair2 &;
...
class topo_dragonfly2 : public Topology {
   public:
    SST_ELI_REGISTER_SUBCOMPONENT(topo_dragonfly2, "merlin", "dragonfly2",
                                  SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                  "Dragonfly2 topology object.  Implements a dragonfly with a "
...
class topo_dragonfly2_event : public internal_router_event {
   public:
    ...
    explicit topo_dragonfly2_event(const topo_dragonfly2::dgnfly2Addr &dest) : dest(dest) {}

    ~topo_dragonfly2_event() override = default;

    auto clone() -> internal_router_event * override { return new topo_dragonfly2_event(*this); }
