...
using port_queue_t = std::queue<internal_router_event *>;
...
class PortControl {
   private:
    ...
    Link *dynlink_timing;
    // Threshold of how idle a link is before it reduces link width 0 to 1 (negative means no link
    // adjustments). i.e. if (idle > dlink_thresh) then reduce link width.
    float dlink_thresh;
   ...
   public:
    ...
    // Returns true if there is space in the output buffer and false
    // otherwise.
    auto spaceToSend(int vc, int flits) -> bool;

    // Returns NULL if no event in input_buf[vc]. Otherwise, returns
    // the next event.
    auto recv(int vc) -> internal_router_event *;

    auto getVCHeads() -> internal_router_event ** { return vc_heads; }

    // time_base is a frequency which represents the bandwidth of the link in flits/second.
    PortControl(Router *rif, int rtr_id, std::string link_port_name, int port_number,
                ...
                std::vector<std::string> &inspector_names, float dlink_thresh, bool oql_track_port,
                bool oql_track_remote);
    ...
    auto recvInitData() -> Event *;

    void sendUntimedData(Event *ev);

    auto recvUntimedData() -> Event *;
    ...
    auto decreaseLinkWidth() -> bool;

    auto increaseLinkWidth() -> bool;

   private:
    std::vector<SST::Interfaces::SimpleNetwork::NetworkInspector *> network_inspectors;

    static void dumpQueueState(port_queue_t &q, std::ostream &stream);

    static void dumpQueueState(port_queue_t &q, Output &out);
