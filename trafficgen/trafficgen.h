...
class TrafficGen : public Component {
   public:
    SST_ELI_REGISTER_COMPONENT(TrafficGen, "merlin", "trafficgen", SST_ELI_ELEMENT_VERSION(1, 0, 0),
                               "Pattern-based traffic generator.", COMPONENT_CATEGORY_NETWORK)
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        {"delay_between_packets", "", "0"}, {"message_rate", "", "1GHz"},
        {"PacketDest:pattern",
         "Address pattern to be used (NearestNeighbor, Uniform, HotSpot, Normal, Binomial)",
         nullptr},
        ...
        {"PacketDest:NearestNeighbor:3DSize",
         "For Nearest Neighbors, the 3D size \"x y z\" of the mesh", ""},
        {"PacketDest:HotSpot:target", "For HotSpot, which node is the target", ""},
        {"PacketDest:HotSpot:targetProbability",
         "For HotSpot, with what probability is the target targeted", ""},
        ...
        {"PacketSize:pattern", "Address pattern to be used (Uniform, HotSpot, Normal, Binomial)",
         nullptr},
        ...
        {"PacketDelay:pattern", "Address pattern to be used (Uniform, HotSpot, Normal, Binomial)",
         nullptr},
   ...
   private:
    ...
    class Generator {
       public:
        virtual auto getNextValue() -> int = 0;
    ...
    class NearestNeighbor : public Generator {
       ...
       public:
            ...
            switch (numNeighbors) {
                ...
                default:
                    Simulation::getSimulation()->getSimulationOutput().fatal(
                        CALL_INFO, -1, "Unsure how to deal with %d neighbors\n", numNeighbors);
            }
        }
        ...
        void seed(uint32_t val) override { dist->seed(val); }
    };
    ...
    class ExponentialDist : public Generator {
       ...
       public:
        explicit ExponentialDist(int lambda) {
            gen = new MersenneRNG();
            dist = new SSTExponentialDistribution(static_cast<double>(lambda));
        }
        ...
        auto getNextValue() -> int override { return static_cast<int>(dist->getNextDouble()); }
    ...
    class DiscreteDist : public Generator {
       ...
       public:
        ...
        auto getNextValue() -> int override {
            return (static_cast<int>(dist->getNextDouble())) + minValue;
        }

        void seed(uint32_t val) override { gen = new MersenneRNG(static_cast<unsigned int>(val)); }
    };
    ...
    class NormalDist : public Generator {
       ...
       public:
        ...
        auto getNextValue() -> int override {
            for (;;) {
                int val = static_cast<int>(dist->getNextDouble());
   ...
   private:
    auto buildGenerator(const std::string &prefix, Params &params) -> Generator *;

    auto clock_handler(Cycle_t cycle) -> bool;

    auto fattree_ID_to_IP(int id) -> int;

    auto IP_to_fattree_ID(int ip) -> int;
   ...
   protected:
    auto getPacketDest() -> int;

    auto getPacketSize() -> int;

    auto getDelayNextPacket() -> int;
};
