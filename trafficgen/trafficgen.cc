...
TrafficGen::TrafficGen(ComponentId_t cid, Params &params)
    : Component(cid),
      //    last_vc(0),
      packets_sent(0), packets_recd(0), done(false), packet_delay(0), packetDestGen(nullptr), packetSizeGen(nullptr),
    ...
    link_control =
        loadUserSubComponent<SST::Interfaces::SimpleNetwork>("networkIF", ComponentInfo::SHARE_NONE, num_vns);
    ...
    link_control->setNotifyOnReceive(
        new SST::Interfaces::SimpleNetwork::Handler<TrafficGen>(this, &TrafficGen::handle_receives));
...
TrafficGen::Generator *TrafficGen::buildGenerator(const std::string &prefix, Params &params) {
    ...
    std::pair<int, int> range =
        std::make_pair(params.find<int>(prefix + ":RangeMin", 0), params.find<int>(prefix + ":RangeMax", INT_MAX));
...
bool TrafficGen::clock_handler(Cycle_t /*cycle*/) {
    if (done)
        return true;
...
bool TrafficGen::handle_receives(int vn) {
...
bool TrafficGen::send_notify(int /*vn*/) {
    reregisterClock(clock_tc, clock_functor);
    return false;
}

int TrafficGen::getPacketDest() {
    int dest = packetDestGen->getNextValue();
    assert(dest >= 0);
    return dest;
}

int TrafficGen::getPacketSize() {
    if (packetSizeGen) {
...
int TrafficGen::getDelayNextPacket() {
    if (packetDelayGen) {
