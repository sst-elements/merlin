...
class offered_load_event : public Event {
  public:
    ...
    offered_load_event(SimTime_t start_time) : Event(), start_time(start_time) {}
...
class offered_load_complete_event : public Event {
  public:
    ...
    offered_load_complete_event(int generation)
        : Event(), generation(generation), sum(0), sum_of_squares(0), min(MAX_SIMTIME_T), max(0), count(0) {}

    ~offered_load_complete_event() override = default;

    offered_load_complete_event *clone() override {
...
class OfferedLoad : public Component {
  ...
  public:
    SST_ELI_REGISTER_COMPONENT(OfferedLoad, "merlin", "offered_load", SST_ELI_ELEMENT_VERSION(0, 0, 1),
                               "Pattern-based traffic generator to study latency versus offered load.",
                               COMPONENT_CATEGORY_NETWORK)

    SST_ELI_DOCUMENT_PARAMS({"num_peers", "Total number of endpoints in network."},
                            {"link_bw",
