...
class single_arb_lru : public SingleArbitration {
  ...
  public:
    SST_ELI_REGISTER_MODULE_DERIVED(single_arb_lru, "merlin", "arb.base.single.lru", SST_ELI_ELEMENT_VERSION(1, 0, 0),
  ...
  public:
    single_arb_lru(Params & /*params*/) : SingleArbitration() {
        Simulation::getSimulationOutput().fatal(CALL_INFO_LONG, 1,
                                                "single_arb_rr: module is only loadable with new API\n");
    }

    single_arb_lru(Params & /*params*/, int16_t size) : SingleArbitration() {
    ...
    ~single_arb_lru() override { delete[] order; }
