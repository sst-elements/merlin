...
bool PortControl::spaceToSend(int vc, int flits) {
    if (xbar_in_credits[vc] < flits)
        return false;
    return true;
}

internal_router_event *PortControl::recv(int vc) {
#if TRACK
    if (rtr_id == TRACK_ID && port_number == TRACK_PORT) {
...
PortControl::PortControl(ComponentId_t cid, Params &params, Router *rif, int rtr_id, int port_number, Topology *topo)
    : PortInterface(cid), rtr_id(rtr_id), num_vcs(-1), topo(topo), port_number(port_number),
      remote_rdy_for_credits(false), input_buf(nullptr), output_buf(nullptr), input_buf_count(nullptr),
      output_buf_count(nullptr), port_ret_credits(nullptr), port_out_credits(nullptr), idle_start(0), sai_win_start(0),
      sai_port_disabled(false), ongoing_transmit(false), is_idle(true), is_active(false), waiting(true),
      have_packets(false), start_block(0), parent(rif), output(Simulation::getSimulation()->getSimulationOutput()) {
    ...
    if (!link_bw.hasUnits("b/s") && !link_bw.hasUnits("B/s")) {
        merlin_abort.fatal(CALL_INFO, -1,
                           "PortControl: link_bw must be specified in either "
                           "b/s or B/s: %s\n",
    ...
    if (!input_buf_size.hasUnits("b") && !input_buf_size.hasUnits("B")) {
        merlin_abort.fatal(CALL_INFO, -1,
                           "PortControl: input_buf_size must be specified in either "
                           "bits (b) or bytes (B): %s\n",
    ...
    if (!output_buf_size.hasUnits("b") && !output_buf_size.hasUnits("B")) {
        merlin_abort.fatal(CALL_INFO, -1,
                           "PortControl: output_buf_size must be specified in either "
                           "bits (b) or bytes (B): %s\n",
    ...
    for (unsigned int i = 0; i < inspector_names.size(); i++) {
        Params empty;
        auto *ni = loadAnonymousSubComponent<SimpleNetwork::NetworkInspector>(
    ...
    output_arb = loadAnonymousSubComponent<OutputArbitration>(output_arb_name, "arbitration", 0,
                                                              ComponentInfo::SHARE_NONE, arb_params);
    ...
    if (num_vns != -1) {
        ...
        if (vn_remap_shm != "") {
            ...
            shared_region = Simulation::getSharedRegionManager()->getGlobalSharedRegion(vn_remap_shm, size,
...
void PortControl::initVCs(int vns, int *vcs_per_vn, internal_router_event **vc_heads_in, int *xbar_in_credits_in,
...
Event *PortControl::recvInitData() { return recvUntimedData(); }
...
Event *PortControl::recvUntimedData() {
...
void PortControl::dumpState(std::ostream &stream) {
    ...
    if (is_idle)
        stream << "Time since last active = " << (Simulation::getSimulation()->getCurrentSimCycle() - idle_start)
               << std::endl;
...
void PortControl::dumpQueueState(port_queue_t &q, std::ostream &stream) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        stream << "      dest = " << ev->getDest() << ", size = " << ev->getFlitCount() << ", vc = " << ev->getVC()
               << ", next_port = " << ev->getNextPort() << std::endl;
        q.pop();
        q.push(ev);
    }
}
...
void PortControl::dumpQueueState(port_queue_t &q, Output &out) {
    ...
    for (int i = 0; i < size; i++) {
        internal_router_event *ev = q.front();
        out.output("      dest = %d, size = %d, vc = %d, next_port = %d\n", ev->getDest(), ev->getFlitCount(),
                   ev->getVC(), ev->getNextPort());
        q.pop();
        q.push(ev);
    }
}
...
void PortControl::handle_input_n2r(Event *ev) {
    ...
    // if ( ce != NULL ) {
    auto *base_event = static_cast<BaseRtrEvent *>(ev);
    ...
    case BaseRtrEvent::PACKET: {
        auto *event = static_cast<RtrEvent *>(ev);
        ...
        if (event->getTraceType() != SST::Interfaces::SimpleNetwork::Request::NONE) {
            output.output("TRACE(%d): %" PRIu64 " ns: Received an event on port %d in router %d"
                          ...
                          event->getTraceID(), getCurrentSimTimeNano(), port_number, rtr_id, getName().c_str(), curr_vc,
                          event->getTrustedSrc(), event->getDest());
...
void PortControl::handle_input_r2r(Event *ev) {
    ...
    case BaseRtrEvent::PACKET:
        ...
        break;
    case BaseRtrEvent::INTERNAL: {
        auto *event = static_cast<internal_router_event *>(ev);
        // Simply put the event into the right virtual network queue
...
void PortControl::handle_output(Event * /*ev*/) {
#if TRACK
    if (rtr_id == TRACK_ID && port_number == TRACK_PORT) {
        printStatus(Simulation::getSimulation()->getSimulationOutput(), 0, 0);
    }
    ...
    if (!sai_port_disabled)
        vc_to_send =
            output_arb->arbitrate(getCurrentSimTime(flit_cycle), output_buf, port_out_credits, host_port, have_packets);
    ...
    if (vc_to_send != -1) {
        ...
        if (send_event->getTraceType() == SimpleNetwork::Request::FULL) {
            output.output("TRACE(%d): %" PRIu64 " ns: Sent and event to router from PortControl in router: %d"
                          " (%s) on VC %d from src %d to dest %d.\n",
                          send_event->getTraceID(), getCurrentSimTimeNano(), rtr_id, getName().c_str(),
                          send_event->getVC(), send_event->getSrc(), send_event->getDest());
        }
        ...
        for (auto &network_inspector : network_inspectors) {
            network_inspector->inspectNetworkData(send_event->inspectRequest());
        }
...
bool PortControl::decreaseLinkWidth() {
...
//
// TODO add delay on port, before it can transmit data again.
bool PortControl::increaseLinkWidth() {
