...
class output_arb_qos_multi : public PortInterface::OutputArbitration {
  ...
  public:
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(output_arb_qos_multi, "merlin", "arb.output.qos.multi",
                                          SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                          "Multi-level output arbitration with quality of service for PortControl",
                                          SST::Merlin::PortInterface::OutputArbitration)

    SST_ELI_DOCUMENT_PARAMS({"arb_vns", "Tyoe of arbitration to use for virtual networks",
                             "merlin.arb.base.single.lru"},
                            {"arb_vcs", "Tyoe of arbitration to use with virtual network for VCs",
                             "merlin.arb.base.single.roundrobin"},
  ...
  public:
    output_arb_qos_multi(ComponentId_t cid, Params &params) : OutputArbitration(cid), last_cycle(0) {
    ...
    void setVCs(int n_vns, int *vcs_per_vn_in) override {
        if (n_vns != num_vns) {
            ...
            merlin_abort.fatal(CALL_INFO_LONG, 1,
                               "output_arb_qos_multi: size of qos_settings does not match number of vns in network\n");
        }
    ...
    int arbitrate(Cycle_t cycle, PortInterface::port_queue_t *out_q, int *port_out_credits, bool isHostPort,
                  bool &have_packets) override {
        ...
        for (int i = 0; i < num_vns; ++i) {
            ...
            for (int j = 0; j < vcs_per_vn[vn]; ++j) {
                ...
                if (counters[which_counter[vn]] <= 0)
                    continue;
                // Check to see if there are enough port credits
                if (port_out_credits[isHostPort ? send_event->getVN() : actual_vc] < send_event->getFlitCount())
                    continue;
