...
class PortControl : public PortInterface {
  public:
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(PortControl, "merlin", "portcontrol", SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                          "Port Control module for use by hr_router", SST::Merlin::PortInterface)
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        {"flit_size", "Size of a flit in either b or B (can include SI prefix)."}, {"input_latency", "", "0ns"},
        {"output_latency", "", "0ns"},
    ...
    SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS({"inspector_slot", "Network inspectors",
                                         "SST::Interfaces::SimpleNetwork::NetworkInspector"},
                                        {"arbitration", "Arbitration unit to use for output",
  ...
  public:
    void sendTopologyEvent(TopologyEvent *ev) override;
    ...
    internal_router_event **getVCHeads() override { return vc_heads; }
    ...
    void initVCs(int vns, int *vcs_per_vn, internal_router_event **vc_heads, int *xbar_in_credits,
                 int *output_queue_lengths) override;

    ~PortControl() override;
    void setup() override;
    void finish() override;
    void init(unsigned int phase) override;
    void complete(unsigned int phase) override;

    void sendInitData(Event *ev) override;
    Event *recvInitData() override;
