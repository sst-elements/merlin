...
ReorderLinkControl::ReorderLinkControl(ComponentId_t cid, Params &params, int vns)
    : SimpleNetwork(cid), receiveFunctor(nullptr), vns(vns) {
    if (isUser()) {
    ...
    if (isUser())
        childParams.insert("port_name", "rtr_port");

    input_buf = new request_queue_t[vns];

    link_control = loadAnonymousSubComponent<SimpleNetwork>(
        networkIF, "networkIF", 0, ComponentInfo::INSERT_STATS | ComponentInfo::SHARE_PORTS, childParams, vns);
}

ReorderLinkControl::~ReorderLinkControl() { delete[] input_buf; }

#ifndef SST_ENABLE_PREVIEW_BUILD
bool ReorderLinkControl::initialize(const std::string & /*port_name*/, const UnitAlgebra & /*link_bw_in*/, int vns,
                                    const UnitAlgebra & /*in_buf_size*/, const UnitAlgebra & /*out_buf_size*/) {
    // if ( !wasLoadedWithLegacyAPI() ) {
    //     merlin_abort.fatal(CALL_INFO_LONG,1,"reorderLinkControl::initializae() was called on instance that was loaded
    //     using new APIs.  This method can only be called when loaded with the legacy API.  Use
    //     wasLoadedWithLegacyAPI() to check load status."); return false;
    // }
    this->vns = vns;
...
void ReorderLinkControl::setup() {
    ...
    link_control->setNotifyOnReceive(
        new SimpleNetwork::Handler<ReorderLinkControl>(this, &ReorderLinkControl::handle_event));
}
...
// Returns true if there is space in the output buffer and false
// otherwise.
bool ReorderLinkControl::spaceToSend(int vn, int bits) { return link_control->spaceToSend(vn, bits); }
...
SST::Interfaces::SimpleNetwork::Request *ReorderLinkControl::recvUntimedData() { return link_control->recvInitData(); }

void ReorderLinkControl::sendInitData(SST::Interfaces::SimpleNetwork::Request *req) { link_control->sendInitData(req); }

SST::Interfaces::SimpleNetwork::Request *ReorderLinkControl::recvInitData() { return link_control->recvInitData(); }
...
bool ReorderLinkControl::isNetworkInitialized() const { return link_control->isNetworkInitialized(); }

SST::Interfaces::SimpleNetwork::nid_t ReorderLinkControl::getEndpointID() const {
    return link_control->getEndpointID();
}

const UnitAlgebra &ReorderLinkControl::getLinkBW() const { return link_control->getLinkBW(); }

bool ReorderLinkControl::handle_event(int vn) {
    auto *my_req = static_cast<ReorderRequest *>(link_control->recv(vn));
